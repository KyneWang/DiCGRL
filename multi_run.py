...
from baselines import cal_ewc_loss, consolidate, estimate_fisher
...
parser.add_argument("--do_ewc", type=int, default=0, help="do ewc baseline")
...
def main():
    ...
    for idx in range(args.s_N, args.N):
        ...
        if args.all_data or args.up_bound:
            train_data, validation_data, test_data, entity2id, relation2id, sub_entity2id, test_sub_triples, valid_triples_list = \
        ...
        train_loader = Corpus(args, train_data, validation_data, test_data, sub_entity2id, relation2id,
                        args.batch_size, args.valid_invalid_ratio, valid_triples_list)
        ...
        file_name = "model_name_" + str(args.model_name) + "_embedding_size_" + str(args.embedding_size) + "_lr_" + str(
            ...
            args.top_n) + "_att_lr_" + str(args.att_lr) + "_do_ewc_" + str(args.do_ewc)
...
def train(args, train_loader, model, model_path, data_idx):
    ...
    for epoch in range(args.epochs):
        ...
        for iters in range(num_iters_per_epoch):
            ...
            if args.do_ewc:
                ewc_loss = cal_ewc_loss(model, 15)
                #print("ewc_loss", ewc_loss)
                att_or_baseline = "Ewc"
                loss = loss + ewc_loss
                other_loss_data = ewc_loss.data.item()
    ...
    if args.do_ewc:
        ...
        fisher = estimate_fisher(model, train_loader, num_iters_per_epoch,
                                                   fisher_estimation_sample_size, batch_size=args.batch_size)
        model = consolidate(model, fisher)
